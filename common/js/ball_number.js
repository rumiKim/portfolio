$(function(){
	...
	var obj_height	= $(".cart_list").find("ol").css("height");
	...
	function button_chk(nm, val,objs){
		...
		if(obj.find("li").eq(nm).find("button").text() != ''){
			return false;
		}
		...
		obj.find("li").eq(nm).find("button").text(val);
		obj.css("height",obj_height);
	...
	$(document).on("click",".btn_del",function(){
		...
		if($("#five_btn").find(".white").length < 5 && $("#five_btn").find(".white").length > 0 && chk != d_num){
			chk--;
		}else if($("#one_btn").find("."+color).length < 1 && $("#five_btn").find(".white").length > 0 && chk != d_num){
			chk--;
		}else if($("#one_btn").find("."+color).length > 0 && $("#five_btn").find(".white").length == 0 && chk != d_num){
			chk--;
		}else{
			$("#five_btn").find(".white").removeClass("white");
			$("#one_btn").find("."+color).removeClass(color);
	...
	$(document).on("click",".btn_type1",function(){
		...
		$(".cart_list").find("ol").each(function(){
			var number = "";
			for(var i=0;i<6;i++){
				if($.trim($(this).find("li").eq(i).find("button").html()) != ''){
